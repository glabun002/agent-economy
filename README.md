...
```
  ...
  MarketPulse    Commerce Feed   own wallet that sells synthesis
                                 work via x402)
...
| Dir | What it is | Port |
|---|---|---|
| `agent/` | **Scout**, the buyer agent. An LLM makes the buy/skip/hire decisions under a hard spend policy; a smart wallet signs the USDC payments via x402. | — |
...
| `dashboard/` | **Mission Control**: live dashboard with budget, decisions, rejections, on-chain receipts, and an animated agent network. | 4030 |
...
`.env.example` files. A full run spends $0.40 and takes 60 to 90 seconds.
...
- **[Claude](https://claude.com)**: the judgment layer, making buy/skip/hire
  decisions under a hard spend policy
